...
export default function AdminEnrichPanel() {
  ...
  async function run() {
    controller.current = new AbortController(); setBusy(true); setMessage("");
    try { await api.enrich(selected, (line) => { const x = line as { status?: string; ergänzt?: number; gesamt?: number; verarbeitet?: number; aktuell?: { titel?: string }; felder?: string[] }; if (x.status === "progress") setMessage(`${x.verarbeitet ?? 0} / ${x.gesamt ?? 0}: ${x.aktuell?.titel ?? "Unbekannter Film"} – ${x.felder?.join(", ") ?? ""}`); if (x.status === "done") setMessage(`Fertig: ${x.ergänzt ?? 0} Einträge ergänzt.`); if (x.status === "aborted") setMessage(`Abgebrochen nach ${x.verarbeitet ?? 0} Filmen.`); }, controller.current.signal); setPreview(await api.enrichPreview()); }
    catch (e) { if (e instanceof DOMException && e.name === "AbortError") setMessage("Abbruch angefordert – der aktuelle Film wird noch beendet."); else setMessage(e instanceof Error ? e.message : "Fehler"); } finally { controller.current = null; setBusy(false); }
  }
  function cancel() { controller.current?.abort(); setMessage("Abbruch angefordert – der aktuelle Film wird noch beendet."); }
  return <section className="admin-enrich"><h2>Daten ergänzen</h2><p>Aktuelle Lücken:</p>{preview ? <div className="enrich-stats">{fields.map((f) => <label key={f.key}><input type="checkbox" checked={selected.includes(f.key)} onChange={() => toggle(f.key)} disabled={busy} />{f.label}: {preview[f.key]}</label>)}</div> : <p>Lade Statistik …</p>}<button className="primary" onClick={() => void run()} disabled={busy || !preview || selected.length === 0}>{busy ? "Ergänze …" : "Auswahl ergänzen"}</button>{busy && <button onClick={cancel}>Abbrechen</button>}{message && <p>{message}</p>}</section>;
}
